...
if [ ! -d "base/Paper/PaperSpigot-Server" ]; then
    echo "Upstream directory does not exist. Did you forget to run 'dash setup'?"
    exit 1
else
    # Apply PandaSpigot
    (
        applyPatch "base/Paper/PaperSpigot-API" PandaSpigot-API HEAD patches/pandaspigot/api &&
        applyPatch "base/Paper/PaperSpigot-Server" PandaSpigot-Server HEAD patches/pandaspigot/server
        cd "$basedir"
    ) || (
	...
	# Apply DashSpigot
    (
        applyPatch PandaSpigot-API DashSpigot-API HEAD patches/dashspigot/api &&
        applyPatch PandaSpigot-Server DashSpigot-Server HEAD patches/dashspigot/server
        cd "$basedir"
    ) || (
        echo "Failed to apply DashSpigot Patches"
        exit 1
    ) || exit 1
fi
) || exit 1
